...
| `__all__ = ["HelloWorld"]` | Lists the module's public name(s) — what `from module import *` would export. AutoPkg doesn't need it (it loads the class by name via `getattr`); shared processors include it by convention to declare their public API. |
...
Or run it for you: `bash stages/step_10_finished_processorstandalone.sh`
